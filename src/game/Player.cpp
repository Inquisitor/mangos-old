...
void Player::RewardReputation(Unit *pVictim, float rate)
{
    ...
    if(Rep->repfaction2 && (!Rep->team_dependent || GetTeam()==HORDE))
    {
        ...
        int32 donerep2 = CalculateReputationGain(REPUTATION_SOURCE_KILL, Rep->repvalue2, realFaction2, pVictim->getLevel());
...
void Player::SendLoot(ObjectGuid guid, LootType loot_type)
{
    ...
    switch(guid.GetHigh())
    {
        case HIGHGUID_GAMEOBJECT:
        {
            ...
            if (!go || (loot_type != LOOT_FISHINGHOLE && (loot_type != LOOT_FISHING && loot_type != LOOT_FISHING_FAIL || go->GetOwnerGuid() != GetObjectGuid()) && !go->IsWithinDistInMap(this,(loot_type == LOOT_PICKPOCKETING && HasAura(58017)) ? 10.0f : INTERACTION_DISTANCE)))
            {
...
bool Player::TakeQuestSourceItem(uint32 quest_id, bool msg)
{
    if (Quest const* qInfo = sObjectMgr.GetQuestTemplate(quest_id))
    {
        if(uint32 srcitem = qInfo->GetSrcItemId())
        {
            ...
            if(uint8 res = CanUnequipItems(srcitem,count) != EQUIP_ERR_OK)
            {
                if(msg)
...
void Player::AdjustQuestReqItemCount(Quest const* pQuest, QuestStatusData& questStatusData)
{
    if (pQuest->HasSpecialFlag(QUEST_SPECIAL_FLAG_DELIVER))
    {
        for(int i = 0; i < QUEST_ITEM_OBJECTIVES_COUNT; ++i)
        {
            if(uint32 reqitemcount = pQuest->ReqItemCount[i])
            {
...
void Player::AreaExploredOrEventHappens(uint32 questId)
{
    if(questId)
    {
        if(uint16 log_slot = FindQuestSlot(questId) < MAX_QUEST_LOG_SIZE)
        {
            QuestStatusData& q_status = mQuestStatus[questId];
...
{
    bool queueOk = true;
    sLog.outBasic("%s::CheckItemSaveQueue() checking item save queue...", GetGuidStr().c_str());
    ...
    sLog.outBasic("%s::CheckItemSaveQueue() checking item save queue - SUCCESS", GetGuidStr().c_str());
    return queueOk;
}
...
void Player::_SaveStats()
{
    ...
    ss << "INSERT INTO character_stats (guid, maxhealth, maxpower1, maxpower2, maxpower3, maxpower4, maxpower5, maxpower6, maxpower7, "
        ...
        "blockPct, dodgePct, parryPct, critPct, rangedCritPct, spellCritPct, spellCritPct1, spellCritPct2, spellCritPct3, spellCritPct4, spellCritPct5, spellCritPct6, attackPower, rangedAttackPower, spellPower, "
        "attackSpeed, offhandAttackSpeed, rangedAttackSpeed, minDamage, maxDamage, minOffhandDamage, maxOffhandDamage, attackPowerMod, attackPowerMultiplier, "
    ...
    for(int i = 0; i < MAX_SPELL_SCHOOL; ++i)
        ss << GetUInt32Value(UNIT_FIELD_RESISTANCES + i) + GetFloatValue(UNIT_FIELD_RESISTANCEBUFFMODSPOSITIVE + i) + GetFloatValue(UNIT_FIELD_RESISTANCEBUFFMODSNEGATIVE + i) << ",";
    ss << GetFloatValue(PLAYER_BLOCK_PERCENTAGE) << ", "
       ...
       << GetFloatValue(PLAYER_SPELL_CRIT_PERCENTAGE1+1) << ", "
       << GetFloatValue(PLAYER_SPELL_CRIT_PERCENTAGE1+2) << ", "
       << GetFloatValue(PLAYER_SPELL_CRIT_PERCENTAGE1+3) << ", "
       << GetFloatValue(PLAYER_SPELL_CRIT_PERCENTAGE1+4) << ", "
       << GetFloatValue(PLAYER_SPELL_CRIT_PERCENTAGE1+5) << ", "
       << GetFloatValue(PLAYER_SPELL_CRIT_PERCENTAGE1+6) << ", "
