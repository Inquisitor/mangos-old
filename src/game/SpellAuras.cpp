        ...
        if( !caster->hasUnitState(UNIT_STAT_ISOLATED) )
        {
            ...
            for(std::list<Unit *>::iterator tIter = targets.begin(); tIter != targets.end(); tIter++)
            {
                ...
                if(SpellEntry const *actualSpellInfo = sSpellMgr.SelectAuraRankForPlayerLevel(GetSpellProto(), (*tIter)->getLevel()))
                {
                    ...
                    {
                        if (sSpellMgr.IsRankSpellDueToSpell(actualSpellInfo, (*itr).second->GetId()))
                        {
                            canAdd = false;
                            break;
                        }
...
bool Aura::isAffectedOnSpell(SpellEntry const *spell) const
{
    ...
    // Check EffectClassMask
     uint32 const *ptr;
     SpellAffectEntry const *spellAffect = sSpellMgr.GetSpellAffect(GetId(), GetEffIndex());
     if (spellAffect)
...
void Aura::HandleAddModifier(bool apply, bool Real)
{
    ...
    if (apply)
    {
        ...
        uint32 const *ptr;
        SpellAffectEntry const *spellAffect = sSpellMgr.GetSpellAffect(GetId(), m_effIndex);
        if (spellAffect)
            ptr = &spellAffect->SpellClassMask[0];
        else
        {
...
void Aura::HandleAuraDummy(bool apply, bool Real)
{
    ...
    else
    {
        ...
        switch(GetId())
        {
            ...
            {
                if (Unit* caster = GetCaster())
                    if( GetTarget() && GetTarget()->GetEntry() == 18879 && (GetTarget()->GetHealth() * 100 / GetTarget()->GetMaxHealth() < 30) )
        ...
        case SPELLFAMILY_PRIEST:
        {
            ...
            if (!apply && GetSpellProto()->SpellFamilyFlags & UI64LIT(0x40000000000) && GetEffIndex() == 2)
            {
                ...
                if (Unit* caster = GetCaster())
                {
                    if ( m_removeMode == AURA_REMOVE_BY_DISPEL )
                    {
                        // amount equal to 24 sec worth of ticks, or 8 ticks
                        int32 amount = 8*caster->SpellDamageBonus(m_target, GetSpellProto(), GetSpellProto()->EffectBasePoints[1], DOT);
        ...
        case SPELLFAMILY_DRUID:
        {
            ...
            if (GetSpellProto()->SpellFamilyFlags & UI64LIT(0x1000000000))
            {
                ...
                else
                {
                    ...
                    if(m_removeMode == AURA_REMOVE_BY_DISPEL && !m_stackAmount)
                        m_stackAmount = 1;
...
void Aura::HandleModPossess(bool apply, bool Real)
{
    ...
    else
    {
        ...
        if(m_target->GetTypeId() == TYPEID_UNIT)
        {
            ...
            m_target->getThreatManager().addThreat(p_caster, 9999999.0f);
...
void Aura::PeriodicTick()
{
    switch(m_modifier.m_auraname)
    {
        ...
        case SPELL_AURA_OBS_MOD_HEALTH:
        {
            ...
            pCaster->ProcDamageAndSpell(m_target, procAttacker, procVictim, PROC_EX_NORMAL_HIT, pdamage, BASE_ATTACK, spellProto);
            break;
        }
...
// Exist more after, need add later
        ...
        case SPELLFAMILY_ROGUE:
        {
            switch (spell->Id)
            {
                ...
                case 51690:
                {
                    if (m_target->hasUnitState(UNIT_STAT_STUNNED) || m_target->HasAuraType(SPELL_AURA_MOD_FEAR) || m_target->HasAuraType(SPELL_AURA_MOD_CONFUSE))
                        return;
