    ...
    else if (spellEntry->SpellFamilyName == SPELLFAMILY_PRIEST && spellEntry->SpellFamilyFlags & UI64LIT(0x0000040000000000))
    {
        if (Aura *dot = GetAura(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_PRIEST, UI64LIT(0x0000040000000000), 0x00000000, casterGUID))
        {
            if(Unit* caster = dot->GetCaster())
            {
                ...
                bp0 = caster->SpellDamageBonus(this, spellEntry, bp0, SPELL_DIRECT_DAMAGE, 1);*/
                // Mangos'
                // use clean value for initial damage
    ...
    switch(dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            if (dummySpell->SpellIconID == 113)
            {
                ...
                basepoints[0] = caster->GetMaxPower(POWER_MANA)* dummySpell->EffectBasePoints[2] / 100;
        ...
        case SPELLFAMILY_PRIEST:
        {
            ...
            switch(dummySpell->Id)
            {
                ...
                case 55680:
                {
                    basepoints[0] = int32(damage * 0.4f);
...
bool Unit::HandleProcTriggerSpell(Unit *pVictim, uint32 damage, Aura* triggeredByAura, SpellEntry const *procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    switch(trigger_spell_id)
    {
        ...
        /*case 63734: // Serendipity loop hack-fix
        {
            if(auraSpellInfo->EffectTriggerSpell[0] == 63734)
                return false;
        }*/
    }
...
void Unit::SendHealSpellLog(Unit *pVictim, uint32 SpellID, uint32 Damage, uint32 OverHeal, bool critical)
{
    ...
    data << uint32(0); // New value for absorb, when is spell absorbed?
...
uint32 Unit::SpellDamageBonusDone(Unit *pVictim, SpellEntry const *spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (bonus)
    {
        if (damagetype == DOT && bonus->dot_damage < 0)
            return pdamage;
        else if(bonus->direct_damage < 0)
            return pdamage;
    }
        ...
        case SPELL_DAMAGE_CLASS_MAGIC:
        {
            ...
            if (pVictim)
            {
                ...
                for(AuraList::const_iterator i = mOverrideClassScript.begin(); i != mOverrideClassScript.end(); ++i)
                {
                    ...
                    switch((*i)->GetModifier()->m_miscvalue)
                    {
                        case  849: if (pVictim->isFrozen() || HasAura(44544)) crit_chance+= 17.0f; break; //Shatter Rank 1
                        case  910: if (pVictim->isFrozen() || HasAura(44544)) crit_chance+= 34.0f; break; //Shatter Rank 2
                        case  911: if (pVictim->isFrozen() || HasAura(44544)) crit_chance+= 50.0f; break; //Shatter Rank 3
    ...
    for(ProcTriggeredList::const_iterator i = procTriggered.begin(); i != procTriggered.end(); ++i)
    {
        ...
        switch(auraModifier->m_auraname)
        {
            ...
            case SPELL_AURA_ADD_CASTER_HIT_TRIGGER:
                DEBUG_FILTER_LOG(LOG_FILTER_SPELL_CAST, "ProcDamageAndSpell: casting spell id %u (triggered by %s maelstrom/intervene aura of spell %u)", spellInfo->Id,(isVictim?"a victim's":"an attacker's"), triggeredByAura->GetId());
