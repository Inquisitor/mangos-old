...
uint32 Unit::DealDamage(Unit *pVictim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const *spellProto, bool durabilityLoss)
{
    ...
    else                                                    // if (health <= damage)
    {
        ...
        // hex and other negative transformed cases
        if(uint32 morphspell = pVictim->getTransForm())
        {
            SpellEntry const *morphspellInfo = sSpellStore.LookupEntry(morphspell);
            if(morphspellInfo)
            {
                if (IsSpellHaveAura(morphspellInfo, SPELL_AURA_MOD_PACIFY_SILENCE))
    ...
    if (!(damageInfo->HitInfo & HITINFO_MISS))
    {
        ...
        for(AuraList::const_iterator i = vDamageShields.begin(); i != vDamageShields.end();)
        {
           if (alreadyDone.find(*i) == alreadyDone.end())
           {
               ...
               damage = SpellDamageBonus(pVictim, i_spellProto, damage, SPELL_DIRECT_DAMAGE);
...
bool Unit::IsImmunedToSpell(SpellEntry const* spellInfo)
{
    ...
    if (spellInfo->SpellFamilyName == SPELLFAMILY_WARRIOR && spellInfo->SpellFamilyFlags == UI64LIT(0x100000000))
        return false;
...
int32 Unit::CalculateSpellDamage(SpellEntry const* spellProto, uint8 effect_index, int32 effBasePoints, Unit const* target)
{
    ...
    if(spellProto->Attributes & SPELL_ATTR_LEVEL_DAMAGE_CALCULATION && spellProto->spellLevel &&
            ...
            spellProto->Effect[effect_index] != SPELL_EFFECT_APPLY_AURA &&
            spellProto->Effect[effect_index] != SPELL_EFFECT_DUMMY) 
...
{
    uint32 x = ACTION_BAR_INDEX_START;
