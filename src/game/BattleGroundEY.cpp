    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        ...
        if (obj)
        {
            ...
            while (j < m_PlayersNearPoint[EY_POINTS_MAX].size())
            {
                ...
                if (!plr)
                {
                    sLog.outDebug("BattleGroundEY: Player (GUID: %u) not found!", GUID_LOPART(m_PlayersNearPoint[EY_POINTS_MAX][j]));
                    ++j;
                    continue;
                }
    ...
    for(uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        ...
        if (obj)
        {
            ...
            while (j < m_PlayersNearPoint[i].size())
            {
                ...
                if (!plr)
                {
                    sLog.outDebug("BattleGroundEY: Player (GUID: %u) not found!", GUID_LOPART(m_PlayersNearPoint[i][j]));
